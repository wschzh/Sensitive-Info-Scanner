...
2. 让全盘扫描更可控：提供“全盘扫描/深度扫描”预设，避免用户无意中启用 OCR、低级规则和图片扫描。
...
|---|---|---|---|---|---|
| normal | 用户选择 | 默认开 | 开 | 开 | 用户选择 |
| full_disk_fast | 严重+高级 | 关 | 关 | 开 | 默认关 |
| deep | 全部 | 开 | 开 | 开 | 开 |

验收标准：

- 选择 `full_disk_fast` 时，默认只扫严重/高级。
- Web 文案提示全盘扫描建议使用快速模式。
...
全盘快速模式默认关闭 OCR。OCR 是慢路径，且全盘图片扫描的误报和耗时不可控。深度扫描模式再启用。
